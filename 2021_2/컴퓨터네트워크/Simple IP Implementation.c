...
int my_ip_output(struct my_ip_header my_iph, struct my_ip_routing_entry * rentry) {
	...
	if (((route_output(my_iph))->next_hop) == NULL) 
		next_hop = (route_output(my_iph))->dest; 
	else 
		next_hop = (route_output(my_iph))->next_hop;
	...
	while(cache_ptr != NULL) {
		...
		if (strcmp(cache_ptr->dest, rentry->next_hop) == 0) {
	...
	*((unsigned short *)buffer_ptr) = htons(my_iph.dest_len); // dest len
	buffer_ptr += sizeof(unsigned short); 


    // !!! Fill the blank (Set dest, source len, source, ttl)
	*((unsigned short*)buffer_ptr) = htons(my_iph.dest); // dest
	buffer_ptr += sizeof(unsigned short);

	*((unsigned short*)buffer_ptr) = htons(my_iph.source_len); // source len
	buffer_ptr += sizeof(unsigned short);

	*((unsigned short*)buffer_ptr) = htons(my_iph.source); // source
	buffer_ptr += sizeof(unsigned short);

	*((unsigned short*)buffer_ptr) = htons(my_iph.ttl); // ttl
	buffer_ptr += sizeof(unsigned short);
...
void processEthernet(unsigned char* buffer) {
	...
	if (ethtype == htons(0xfffe))
		buffer_ptr += sizeof(eh);
		my_ip_receive(buffer_ptr);
}
